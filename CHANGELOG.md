...
- JA4db integration has been removed because the upstream service is no longer
  ...
  and local JA4db table are removed.
- The experimental JA4 report has been removed while its design is reconsidered.
  JA4 values now use the normal event search links.
...
- New `evebox pcap extract` command extracts packets from capture spools by
  time range and libpcap filter expression. A port from dumpy.
- New `evebox pcap purge` command removes old capture files by retained file
  count or total size, with dry-run and periodic operation support. A port from
  dumpy.
...
- Elasticsearch versions older than 8.19.0 now log/display a warning that
