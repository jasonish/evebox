...
//!
//! Events are partitioned by day based on the timestamp field (nanoseconds since epoch).
...
impl PartitionManager {
    ...
    /// Generate the partition name for a given nanosecond timestamp.
    pub fn partition_name(ts_nanos: i64) -> String {
        ...
        format!("events_{}", dt.format("%Y_%m_%d"))
    }

    /// Ensure a partition exists for the given timestamp.
    /// Uses an in-memory cache to avoid repeated database calls.
    pub async fn ensure_partition(&self, ts_nanos: i64) -> Result<String> {
        let partition_name = Self::partition_name(ts_nanos);
    ...
    pub async fn ensure_partitions_for_batch(&self, timestamps: &[i64]) -> Result<HashSet<String>> {
        let mut partitions_needed: HashSet<String> = HashSet::new();
        let mut partitions_to_create: Vec<i64> = Vec::new();
