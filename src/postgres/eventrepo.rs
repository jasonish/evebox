...
impl PostgresEventRepo {
    ...
    pub(crate) async fn earliest_timestamp(&self) -> Result<Option<DateTime>> {
        ...
        if let Some(ts) = result {
            Ok(Some(DateTime::from_nanos(ts)))
        } else {
            Ok(None)
        }
    }
    ...
    pub async fn get_event_by_id(&self, event_id: String) -> Result<Option<serde_json::Value>> {
        let rowid: i64 = match event_id.parse() {
            Ok(id) => id,
            Err(_) => return Ok(None),
        };

        let sql = r#"
            SELECT rowid, archived, escalated, source, history
            FROM events
            WHERE rowid = $1
        "#;
        ...
        if let Some(row) = row {
            ...
            let escalated: i32 = row.try_get("escalated")?;
    ...
    pub async fn escalate_event_by_id(&self, _event_id: &str) -> Result<()> {
        todo!("PostgresEventRepo::escalate_event_by_id")
    }

    pub async fn deescalate_event_by_id(&self, _event_id: &str) -> Result<()> {
        todo!("PostgresEventRepo::deescalate_event_by_id")
    }
                    ...
                    for el in &elements {
                        match &el.value {
                            queryparser::QueryValue::String(s) => {
                                ...
                                if el.negated {
                                    builder
                                        .push_where(format!("source_values NOT ILIKE {}", p))
                                        .push_arg(format!("%{}%", s))?;
                                } else {
                                    builder
                                        .push_where(format!("source_values ILIKE {}", p))
                                        .push_arg(format!("%{}%", s))?;
                                }
                            }
            ...
            if let Some(entry) = events.get_mut(&key) {
                ...
                let alert = AggAlert {
                    ...
                    metadata: AggAlertMetadata {
                        count: 1,
                        escalated_count: if escalated > 0 { 1 } else { 0 },
    ...
    async fn set_escalation_by_alert_group(
        ...
        // $1: escalated value (1 or 0)
        add_arg!(if escalate { 1i32 } else { 0i32 });
        ...
        // $3: current escalated value (0 if escalating, 1 if deescalating)
        add_arg!(if escalate { 0i32 } else { 1i32 });
    ...
    pub async fn comment_event_by_id(
        ...
        todo!("PostgresEventRepo::comment_event_by_id")
    }
    ...
    pub async fn agg(
        ...
        if field == "dns.rrname" {
            ...
            builder.select(format!("COUNT({json_path}) AS count"));
        ...
        let sql = format!(
            "{} GROUP BY agg HAVING COUNT(*) > 0 ORDER BY count {} LIMIT {}",
        ...
        while let Some(row) = rows.try_next().await? {
            let count: i64 = row.try_get("count")?;
            if count > 0 {
                let val: String = row.try_get("agg")?;
    ...
    pub async fn agg_stream(
        &self,
        field: &str,
        size: usize,
        order: &str,
        query: Vec<queryparser::QueryElement>,
        tx: Option<tokio::sync::mpsc::UnboundedSender<serde_json::Value>>,
    ) -> Result<Vec<serde_json::Value>> {
        let mut builder = EventQueryBuilder::new();
        builder.select("timestamp");

        // Build the JSON path expression for the field
        let json_path = if field == "dns.rrname" {
            "COALESCE(source->'dns'->'queries'->0->>'rrname', source->'dns'->>'rrname')".to_string()
        } else {
            Self::field_to_json_path(field)
        };

        builder.select(format!("{json_path} AS agg"));
        builder.from("events");

        // Use the actual expression in WHERE, not the alias (PostgreSQL requirement)
        ...
        while let Some(row) = rows.try_next().await? {
            ...
            let agg: String = row.try_get("agg")?;
            let entry = results.entry(agg).or_insert(0);
            *entry += 1;

            // Send intermediate updates every second
            if now.elapsed() >= std::time::Duration::from_secs(1) {
                let rows: Vec<serde_json::Value> = Self::to_sorted_vec(&results, order)
                    .iter()
                    .take(size)
    ...
    /// Helper to sort aggregation results.
    fn to_sorted_vec<'a>(results: &'a std::collections::HashMap<String, i64>, order: &str) -> Vec<(&'a String, &'a i64)> {
    ...
    pub async fn get_sensors(&self) -> Result<Vec<String>> {
        ...
        let cutoff = DateTime::now().datetime - chrono::Duration::hours(24);
        let cutoff_nanos = DateTime::from(cutoff).to_nanos();
